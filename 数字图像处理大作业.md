...
\end{align}
$$
若不存在匹配，则将最不可能的分布（即$\frac{|x_t-\mu_k^t|}{\sigma_k^t}最大$）替换为新分布$\mu_K^t = x_t, \Sigma_K^t = \sigma^2I$，权值变为一个很低的值（$1/N$，$N$为已采样数）。在算法初始化时，设第一个分布为$N(x_1,\sigma^2I)$，权值设为1。算法仅需设置阈值$T$与$K$。
